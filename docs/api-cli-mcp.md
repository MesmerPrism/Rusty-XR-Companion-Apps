...
| CLI | Windows Companion | `doctor`, `devices`, `catalog verify`, `broker status`, `broker compare` |
...
compare, media inspection, and Android phone agent commands, the server returns
an operation plan marked as blocked instead of executing the command.
